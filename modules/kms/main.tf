...
resource "aws_kms_key" "this" {
  ...
  policy                             = data.aws_iam_policy_document.k_policy.json
#  policy                             = coalesce(var.policy, data.aws_iam_policy_document.k_policy[0].json)
#  depends_on = [aws_iam_role.k_role]
...
resource "aws_kms_external_key" "this" {
  ...
  multi_region                       = var.multi_region
#  policy                             = coalesce(var.policy, data.aws_iam_policy_document.k_policy[0].json)
  policy                             = coalesce(var.policy, data.aws_iam_policy_document.k_policy.json)
...
# Policy
################################################################################
data "aws_iam_policy_document" "k_policy" {
